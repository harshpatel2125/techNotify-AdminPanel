<?php
	...
	$qry1 = "SELECT * FROM posts_tbl getLastRecord ORDER BY id DESC LIMIT 1  ";
	$rs1 = mysqli_query($conn,$qry1);

	$qry2 = "SELECT * FROM likes_tbl getLastRecord ORDER BY id DESC LIMIT 1 ";
	$rs2 = mysqli_query($conn,$qry2);

	$qry3 = "SELECT * FROM users_tbl getLastRecord ORDER BY id DESC LIMIT 1  ";
	$rs3 = mysqli_query($conn,$qry3);

	$qry4 = "SELECT * FROM comments_tbl getLastRecord ORDER BY id DESC LIMIT 1  ";
			...
			<div class="row">
				<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
					...
					<section class="main-section" style="padding: 30px;">
									...
									<div class="widget-text">
										<?php
											if (mysqli_num_rows($rs1)>0) 
											{
												...
												{
										?>
										<h4 class="mb-1"><?php echo $row['id']; ?></h4>
										<?php
												}
											}
										?>
						...
						<h5 style="margin-bottom: 30px; margin-top: 30px;" class="mt-4">Edit note...</h5>
						<form method="POST">
							<?php
								$id = $_GET['id'];

								$qry = "SELECT * FROM todolist_tbl WHERE id='$id'";
							...
							<div class="form-group">
								...
								<input type="text" name="note-title" value="<?php echo $row['title'] ?>" placeholder="Enter the Title of Note" class="form-control theme-txtbox" required>
							</div>
							<div class="form-group">
								<label class="form-label">Note</label>
								<textarea name="note-description" placeholder="Enter the Note" class="form-control theme-txtbox mb-3" required><?php echo $row['note'] ?></textarea>
							</div>
							<div class="form-group mb-0 mt-2 text-center">
								<button type="submit" name="submit" class="btn theme-btn mr-2">Save</button>
								<button onclick="location.href='dashboard' " class="btn theme-btn-red ml-2">Cancel</button>
							</div>
							<?php
								...
								if (isset($_POST['submit'])) 
								{
									...
									{
										echo '<script> window.location.href = "todolist-edit" </script>';
									}
								}
							?>
						</form>
